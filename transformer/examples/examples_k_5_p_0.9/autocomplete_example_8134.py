...
_op_op_rewrite = nullptr;
  const string kNewOp = "NoOp";
  TF_RETURN_IF_ERROR(
      tensorflow::AttrTypeMapForOp(kNewOp.c_str(), &types, &orig_op_op_rewrite));
  TF_RETURN_IF_ERROR(
       tensorflow::AttrTypeMapForOp(kNewOp.c_str(), &types, &executor_));
  return Status::OK();
}

}  // namespace tensorflow

int TestEagerOpRewrite::num_eager_e
