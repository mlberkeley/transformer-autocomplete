...
##################################################
 ...
 class is used by the eager rewrite pass.
class EagerOpRewriteRegistration : public EnvRegistration {
 public:
  // Env registration is used for the Eager op rewrites.
  explicit EagerOpRewriteRegistration(const Env* env) : EnvRegistration(env) {}

  // Register the op rewrite pass for a given phase.
  Status Register(Env phase, EagerOperation* orig_op,
                   std::unique_ptr<tensorflow::EagerOpRewrite>* out_op) {
    const std::unordered_map<string, std::unique_ptr<EagerOpRewrite>, kNumPhases>& rewrite_;
    rewrite_->SetProto(&proto_);
    if (!rewrites_) {
