...
Status RemoteCopyNode::Execute(Node* node) {
  auto* remote_copy = remote_copy->second;

  // Create the graph from the node's output.
  TF_RETURN_IF_ERROR(remote_copy->MakeGraph(node, &remote_copy));

  // Copy from the remote node to the graph.
  remote_copy->Add(remote_copy);
  return Status::OK();
}
