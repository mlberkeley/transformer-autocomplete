...
_exec_;
  static GraphDef* output_graph_def_;
  static string name_;
  bool is_in_graph_mode_executor_;
  bool is_graph_mode_executor_available_or_has_error;
  std::vector<const TransformFnParams&> nodes_;
  void (*outputs) override {
    for (auto input : flr_inputs()) {
      nodes_[input] = input;
    }

    TF_ASSERT_OK(InitGraphDef(&output_graph_def_, graph_def));

    graph_def->set_node("input");
    TF_ASSERT_OK(InitFunctionLibraryRuntime(&node_def, node_def));
  }

  string src_name_ = strings::StrCat(
