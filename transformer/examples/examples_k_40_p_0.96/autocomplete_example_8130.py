...
##################################################
                         ...
                         const gtl::InlinedVector<TensorHandle*, 4>& handle);

  TensorHandle* tensor_handle(int num_outputs) {
  if (op == kNoOp) {
    // Note that the tensor_handle's device name means that 'output_idx'
    // is defined in this graph and 'num_outputs' is also defined in the graph.
    if (!TensorHandle->IsMutable(tensor_handle)) {
      return kNoOp;
    }
    tensor_handle = new tensor_handle;
  }

  return tensor_handle;
}

// Utility function executed while this callback is active.
Status Execute(std::function<void
