...
##################################################
 ...
 private:
  string ctx_name_;
  EagerContext* ctx_context_;
};

class EagerTensorHandleHelper {
 public:
  ~EagerTensorHandleHelper();
  explicit EagerTensorHandleHelper(EagerOperation* op);

  EagerTensorHandleHelper(EagerContext* ctx_ctx);
};

inline void Prepare(EagerContext* ctx, Env* env) final {
    env->eigen_device<CPUDevice>() = ctx_->eigen_cpu<CPUDevice>();
    env->eigen_device_map<Eigen::Tensor>() = env_->eigen_cpu_map<Eigen::Tensor>();
    if (op->op == "Eager") {
      ctx_->eigen_context
