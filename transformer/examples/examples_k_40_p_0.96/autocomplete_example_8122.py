...
TEST_F(ShapeInferenceTest, InferenceBackpropShapes) {
  // Shape inference will work as seen in
  const std::vector<tensorflow::Scope::Indexes>& indices;
  TF_ASSERT_OK(InferenceContext::Run({}, indices, {1, 2, 3}))
              .Input({"i", indices});
  ShapeInferenceContext context(0);

  TF_ASSERT_OK(InferenceContext::Create({}}));

  int output_size = 0;
  // Numerics from ndef to output tensor.
  int nsize
