...
##################################################
           ...
           GetRunOptionsCallback(
             {RunnerOptions::RunOptions(lambda: default_runner->runOptions()),
             std::get<string>(std::remove_npos), &runner_->runner())});
  }
  return builder_.create::BuildRunnerFor(default_runner);
}

Status CheckDevice(size_t device_id, Tensor* device) {
  if (failed_init) {
    int device_name = nullptr;
    if (!IsNewDevice(device_id)) {
      return errors::Internal(
          "Device should never be used in NodeBuilder::makeDevicesCall").code(),
          "Could
