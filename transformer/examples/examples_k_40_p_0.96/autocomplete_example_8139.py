...
out, kNodeNameLambda, "\n");
    strings::StrAppend(&out, kNodeNameUtilsLambda, "DebugString");
    string DebugStringLine() const override {
      DebugStringLine name_line = out;
    }
  }

  Status Return();
  int64_t Status() const override { return handle_exec->port(); }

 protected:
  Status Initialize;
  EagerContext* ctx_;
  const DeviceMemory<std::complex<float>>& handle_exec_;
  std::tuple<EagerContext*, 4> input_context_;
  TF_RETURN_IF_ERROR(TensorHandleInitialize(&ctx_, input_context));
...
out, " src_tensor: ", src_->DebugString());
...
#endif  // TENSORFLOW_CORE_COMMON_RUNTIME_EAGER_COPY_TO_DEVICE_NODE_H