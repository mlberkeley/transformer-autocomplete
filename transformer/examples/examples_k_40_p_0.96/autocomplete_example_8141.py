...
Var(env_var_name, &val)) {
    return default_val.zero_value();
  }
  return true;
}

bool CheckEnvVarFromVar(StringPiece env_var_name, const string& var) {
  return true;
}

Status ReadNestingStateFromVar(const string& var) {
  // TODO(sibsonv): make it a bug to use this instead of a null or mutable variable
  Graph *graph;
  string data = GetGraph()->GetAttr("data", &data);
  for (size_t i = 0; i < graph->dims(); ++i) {
    Graph* graph = *graph;
    auto* output_attr = graph->add_attr("data", &output_attr);
    if (output_attr.value().empty()) {
