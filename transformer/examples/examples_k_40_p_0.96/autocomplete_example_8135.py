...
##################################################
 ...
 class is responsible for running this rewrite registry.
class EagerRewriteRegistry {
 public:
  // Adds a copy of a rewrite registry registry in the given context,
  // that allows the run of a single run by eager op rewrite pass.
  static EagerOpRewriteRegistry* CreateRegistry(
      const PluginEagerRewriter& eager_op_rewriter, PluginRewriter* plugin) {}

 private:
  string Name() const = default_;
  EagerOpRewriteRegistration* RegisterRegistry() const { return reinterpret_cast<EagerOpRewrite*>(plugin); }

  // If any op is not set already:
  // Set an auto reset.
  auto SetOpresetRegistry() const {
    auto* reinterpret_cast = reinterpret_cast<const PluginEagerRewriter*>(plugin);
...
##################################################
 ...
 class is used to register a new Eager Op rewrite.
