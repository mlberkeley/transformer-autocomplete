...
, Node* task_device, bool should_run,
      bool should_deleter) = 0;

  // Call is run within the context of the thread.
  // If `host` is null, the caller cannot pass on
  // `task_device` into any `worker` threads (currently there will be
  // only one thread on this thread).
  virtual Status CheckExecutor(bool* task_device,
                             const FunctionLibraryRuntime* kernel_lib) = 0;
  virtual Status ProcessOrWorker(StringPiece c, const string& task_def, string* worker_p,
                                const void* worker
