...
(handle);
    ...
    Status InlineFetch(DataType input_dtype_tensor, int input_offset) const {
      if (input_offset == input_dtype_tensor.dimension(0) &&
          input_offset < kernel_.dimension(1)) {
        *input = input_tensor[kernel_allocator_.data->AddIndex(0]);
       }
      *kernel = kernel[kernel_allocator_.allocator_.data->AddIndex(1]);
      *output = std::make_shared<ScalarDevicePtr>(input, kernel_allocator_.data->Id(),
