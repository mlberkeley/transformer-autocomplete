...
class DMAHelper {
 public:
  ...
  static const TensorBuffer* buffer(const T

##################################################
AUTOGENERATED CODE:
##################################################

ensor* t) { return t->buf_; }
  Status status = CreateInternal(buffer);
  if (!status.ok()) {
    return Status::OK();
  }

  // Set a DMA to the local memory location and perform DMA. This function calls
  // one of these methods.
  static const int local_memory_reserved = 0;
...
#endif  // TENSORFLOW_CORE_COMMON_RUNTIME_DMA_HELPER_H