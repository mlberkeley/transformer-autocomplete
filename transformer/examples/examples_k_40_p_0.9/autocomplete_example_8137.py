...
##################################################
 ...
 OpKernel {
 public:
  AsyncEagerNode() : OpKernel(const EagerContext& context) {
    const auto& target_tensor = context.target->FindTensor();
    for (int i = 0; i < context.device_size; ++i) {
      const Tensor& expected_output_tensor = context.device_size();
      auto handle = ops::Variable(context.inputs[i], TensorShape({i}));
      if (!handle) {
        return errors::Internal("No tensor");
       }
    }
    if (handle) {
      SetIsStatus(errors::Internal("Invalid tensor");
       return IsStatus(errors::Internal("
