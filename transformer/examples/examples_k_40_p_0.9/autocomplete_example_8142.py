...
_list, 0);
  ASSERT_EQ(TF_ATTR_STRING, t);
  ASSERT_NE(m->end(), t->find("transpose_a_list"));
  ASSERT_NE(m->end(), "malloc_a");
  EXPECT_FALSE(s.ok()) << s;
}

// Check that the following attr types are properly mapped.
TF_CHECK_OK(InitFunctionLibraryRuntime(OpRegistry::Global(),
                                                 {{"FunctionName", "MatMul"}}, false));
...
TEST(AttrTypeMap, CacheKey) {
  ...
  ASSERT_FALSE(cache_key == a.Cache