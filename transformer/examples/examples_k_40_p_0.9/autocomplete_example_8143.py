...
##################################################
 ...
 to be registered with a TensorFlow operation named
// `op_name`.
// Note: the op_name passed here may or may not be used directly by
// this kernel, even though all operations using it use it. It is recommended that
// the op_name be used in conjunction with 'is_list' and 'is_function',
// which is recommended to be used directly when loading a TensorFlow operation.
class KernelAndDevice::Init : public OpsWithKernelBase {
 public:
  KernelAndDevice(OpKernel* kernel, const TensorFlowOperationAttributes& attr_attrs);
  explicit KernelAndDevice(OpKernel* kernel, TensorFlowOperationAttributes attr_attrs);
  const TensorFlowOperationAttributes& attr_attrs_;

  ~KernelAndDevice() override {}

 protected:
  void SetKernelAttrsToAttrs() override;
